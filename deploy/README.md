...
За замовчуванням `payment_config_mode=strict`: неповні production-реквізити будь-якого платіжного провайдера блокують активацію релізу. Для контрольованого релізу, коли необов'язковий провайдер свідомо ще не підключений, можна вручну обрати `payment_config_mode=advisory`. Діагностика все одно виконується і показує проблему в логах, але не блокує оновлення решти сайту. Не використовуйте цей режим, якщо незавершений провайдер має бути доступним покупцям.

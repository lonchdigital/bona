                            ...
                            <div class="user-orders">
                                ...
                                @foreach( $userOrders as $order )
                                    ...
                                    <table class="table table-hover order-items">
                                        ...
                                        <tbody>
                                        @foreach($orderProductGroups[$order->id] as $group)
                                            @if($group['is_bundle'])
                                                <tr class="admin-order-bundle__heading"><td colspan="7"><strong>{{ trans('base.cart_bundle_label') }}</strong><span>{{ $group['parent']->name }}</span><small>{{ trans('base.cart_bundle_hint') }}</small></td></tr>
                                            @endif
                                            @foreach(collect([$group['parent']])->concat($group['items']) as $product)
                                                ...
                                                <tr @class(['admin-order-product-row', 'is-bundle-parent' => $group['is_bundle'] && ! $isBundleItem, 'is-bundle-item' => $isBundleItem])>
                                                    <td class="product-image"><a href="{{ route('store.product.page', ['productSlug' => $product->slug]) }}"><img class="order-product-image" src="{{ $imageUrl }}" alt="{{ $product->name }}"></a></td>
                                                    ...
                                                    <td>@if($bundleCategory)<small class="admin-order-product-category">{{ $bundleCategory }}</small>@endif<a href="{{ route('store.product.page', ['productSlug' => $product->slug]) }}">{{ $product->name }}</a></td>

...
@section('content')
    <article class="bona-content-page bona-article-page">
        ...
        <header class="bona-article-hero">
            ...
            @if($blogArticle->hero_image_url)
                <figure class="bona-shell bona-article-hero__media">
                    <img src="{{ $blogArticle->hero_image_url }}" alt="{{ $blogArticle->name }}" width="1440" height="900" fetchpriority="high" decoding="async">
                </figure>
            @endif
        </header>

        <div class="bona-shell bona-article-layout">
            <aside class="bona-article-share" aria-label="{{ trans('base.article_share') }}">
                <span>{{ trans('base.article_share') }}</span>
                <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode($articleUrl) }}" target="_blank" rel="noopener nofollow">Facebook</a>
                <a href="https://t.me/share/url?url={{ urlencode($articleUrl) }}&text={{ urlencode($blogArticle->name) }}" target="_blank" rel="noopener nofollow">Telegram</a>
                <button type="button" class="js-article-share-copy" data-url="{{ $articleUrl }}" data-copied-text="{{ trans('base.article_link_copied') }}">{{ app()->getLocale() === 'ru' ? 'Копировать ссылку' : 'Копіювати посилання' }}</button>
            </aside>

            <div class="bona-article-content">
                @foreach($blogArticle->blocks as $block)
                    @php($content = is_array($block->content) ? $block->content : [])
                    ...
                    @elseif($block->type_id === \App\DataClasses\BlogArticleBlockTypesDataClass::TYPE_QUESTIONS_AND_ANSWERS)
                        ...
                        @if($questions->isNotEmpty())
                            <section class="bona-article-block bona-article-faq">
                                <p class="bona-content-kicker">FAQ</p>
                ...
                @if($authorName)
                    <footer class="bona-article-author">
                        @if($authorAvatar)<img src="{{ $authorAvatar }}" alt="{{ $authorName }}" width="96" height="96" loading="lazy">@endif
                        <div>
                            <span>{{ trans('base.author') }}</span>
                            <strong>@if($authorPageUrl)<a href="{{ $authorPageUrl }}" rel="author">{{ $authorName }}</a>@else{{ $authorName }}@endif</strong>
                            @if($authorJobTitle)<p>{{ $authorJobTitle }}</p>@endif
                        </div>
                    </footer>
                @endif
            </div>

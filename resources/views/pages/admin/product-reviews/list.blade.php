...
@section('content')
    <div class="container-fluid">
        <div class="row justify-content-center">
            <div class="col-12">
                <h2 class="mb-2 page-title">{{ trans('admin.product_reviews') }}</h2>

                <p class="text-muted">{{ trans('admin.product_reviews_hint') }}</p>

                <div class="row mb-3">
                    <div class="col-md-12">
                        <a href="{{ route('admin.product-review.list.page') }}"
                        ...
                        @foreach(\App\DataClasses\ProductReviewStatusesDataClass::get() as $status)
                            <a href="{{ route('admin.product-review.list.page', ['status' => $status['id']]) }}"
                        ...
                        <div class="card shadow">
                            <div class="card-body">
                                ...
                                @if(!count($reviewsPaginated))
                                    <p class="mb-0">{{ trans('admin.product_reviews_empty') }}</p>
                                @else
                                    <table class="table">
                                        <thead>
                                        <tr>
                                        ...
                                        @foreach($reviewsPaginated as $review)
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @if($review->product)
                                                        <a href="{{ route('admin.product.edit.page', ['product' => $review->product->id]) }}" target="_blank">
                                                            {{ $review->product->name }}
                                                        </a>
                                                    @else
                                                        —
                                                ...
                                                <td style="max-width: 340px; white-space: normal;">{{ $review->review }}</td>
                                                <td>
                                                ...
                                                <td class="text-right" style="white-space: nowrap;">
                                                    @if($review->status_id !== \App\DataClasses\ProductReviewStatusesDataClass::STATUS_APPROVED)
                                                        <form action="{{ route('admin.product-review.approve', ['productReview' => $review->id]) }}" method="POST" class="d-inline">
                                                            @csrf
                                                    ...
                                                    @if($review->status_id !== \App\DataClasses\ProductReviewStatusesDataClass::STATUS_REJECTED)
                                                        <form action="{{ route('admin.product-review.reject', ['productReview' => $review->id]) }}" method="POST" class="d-inline">
                                                            @csrf
                                                            <button type="submit" class="btn btn-sm btn-secondary">{{ trans('admin.product_review_reject') }}</button>
                                                        </form>
                                                    @endif
                                                    <form action="{{ route('admin.product-review.delete', ['productReview' => $review->id]) }}" method="POST" class="d-inline">
                                                        @csrf

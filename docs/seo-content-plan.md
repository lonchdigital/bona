...
Для **товарів, категорій і посадкових сторінок груп фільтрів** імпорт уже готовий. Для сторінок послуг окремого імпорту поки немає (див. [розділ 6](#6-що-потребує-доробки-коду-перед-наповненням)).
...
Для груп фільтрів JSON-файли зберігаються в `database/content/filter-groups/`, а ручний запуск виконується командою:

```bash
php artisan filter-groups:apply-content 2026_09_26_interior_door_styles.json
```

Імпортер створює або оновлює саму групу, її фільтр, meta, SEO-текст і FAQ. Усі ці дані після імпорту доступні для редагування в адмінці → SEO → «Групи фільтрів».
...
Стан на 26.09.2026: окремі посадкові сторінки та двомовні тексти створені для сірого, антрациту, айворі й декору «Бетон сірий». Старі URL фільтрів отримують canonical на відповідну посадкову сторінку. Білий колір свідомо не дублюється до рішення щодо двох наявних сторінок «Білі двері».
...
Стан на 26.09.2026: головна сторінка послуг, консультація, виклик майстра на замір і монтаж наповнені українською та російською. Кожна сторінка має 400–700 видимих слів разом із 5 FAQ, окремі title та meta description. Заголовок, вступ, повний текст і FAQ головної сторінки, а також усі поля кожної послуги зберігаються в БД і редагуються в адмінці «Послуги».
...
Стан на 26.09.2026: підготовлено окремі природні російські версії для перших 10 із 20 статей — гіда з вибору міжкімнатних дверей, огляду бренду Danapris, матеріалу про Comeo Porte, гіда з вибору білого RAL 9003, огляду Estet Doors, огляду PORTA NOVA, перевірки пропозицій DiPortes, гіда з вибору ручки DnD Airone, матеріалу про єдиний комплект дверей замість непідтвердженої колекції Bona Dea та перевірки бренду Amega. Перекладені назви, власні RU-slug, анонси, повні тексти, meta title, meta description і по 5 FAQ; усе зберігається у звичайних полях статті та редагується в адмінці блогу. Файли `database/content/blog/2026_09_26_blog_ru_batch_01.json` — `database/content/blog/2026_09_26_blog_ru_batch_05.json`.
...
Стан на 26.09.2026: власний SEO-текст і FAQ для груп фільтрів, їх виведення на фронті та окремий імпортер реалізовані. На цій основі створено чотири сторінки стилів; наповнення кольорових сторінок лишається наступним контентним блоком.
...
- **4 посадкові сторінки стилів міжкімнатних дверей:** «Модерн», «Класика», «Неокласика» і «Мінімалізм» отримали окремі фільтри, title, meta, тексти по 300–500 слів та по 5 FAQ українською і російською. Файл `database/content/filter-groups/2026_09_26_interior_door_styles.json`; сторінки зберігаються в БД, редагуються в адмінці груп фільтрів, мають власні canonical URL, додані до sitemap і ведуть із карток стилів у меню.
- **4 кольорові посадкові сторінки міжкімнатних дверей:** сірий, антрацит, айворі та «Бетон сірий» отримали окремі title, meta й тексти по 150–300 слів українською та російською. Файл `database/content/filter-groups/2026_09_26_interior_door_colors.json`; сторінки зберігаються в БД, редагуються в адмінці, додані до sitemap, а старі фільтрові URL канонікалізуються на них. Білий колір відкладено до вибору головної з двох наявних сторінок.
- **4 сторінки послуг:** `/services`, консультація, виклик майстра на замір і монтаж отримали двомовні тексти на 400–700 видимих слів, по 5 FAQ, title та meta description. Файл `database/content/services/2026_09_26_services_content.json`; увесь контент і FAQ зберігаються в БД, редагуються в адмінці «Послуги» та виводяться на фронті зі структурованими даними FAQPage.
- **Російські версії статей блогу, перша група:** для гіда з вибору міжкімнатних дверей і огляду Danapris додано окремі RU-slug, анонси, повні тексти, meta та по 5 FAQ. Файл `database/content/blog/2026_09_26_blog_ru_batch_01.json`; контент зберігається в БД, редагується у звичайній адмінці статей і не змінює українські версії. Наповнено 2 із 20 статей цього розділу.
- **Російські версії статей блогу, друга група:** для матеріалів про Comeo Porte та білі двері RAL 9003 додано окремі RU-slug, анонси, повні тексти, meta та по 5 FAQ. Файл `database/content/blog/2026_09_26_blog_ru_batch_02.json`; контент зберігається в БД, редагується у звичайній адмінці статей і не змінює українські версії. Загалом наповнено 4 із 20 статей цього розділу.
- **Російські версії статей блогу, третя група:** для оглядів Estet Doors і PORTA NOVA додано окремі RU-slug, анонси, повні тексти, meta та по 5 FAQ. У матеріалі PORTA NOVA виправлено фактологію: Verona 2 визначено як декор, а не окремий бренд. Файл `database/content/blog/2026_09_26_blog_ru_batch_03.json`; контент зберігається в БД, редагується у звичайній адмінці статей і не змінює українські версії. Загалом наповнено 6 із 20 статей цього розділу.
- **Російські версії статей блогу, четверта група:** для матеріалів DiPortes та Airone додано окремі RU-slug, анонси, повні тексти, meta та по 5 FAQ. Виправлено фактологію: актуальна публічна вітрина DiPortes представляє меблі й не підтверджує описані раніше дверні колекції, а Airone визначено як модель латунної ручки DnD дизайну Mauro Ronchi, а не бренд дверей. Файл `database/content/blog/2026_09_26_blog_ru_batch_04.json`; контент зберігається в БД, редагується у звичайній адмінці статей і не змінює українські версії. Загалом наповнено 8 із 20 статей цього розділу.
- **Російські версії статей блогу, п’ята група:** для матеріалів Bona Dea та Amega додано окремі RU-slug, анонси, повні тексти, meta та по 5 FAQ. Виправлено непідтверджену фактологію: у чинному каталозі Bona Doors немає колекції Bona Dea або виробника міжкімнатних дверей Amega, тому тексти перероблено на практичний гід зі створення єдиного дверного комплекту та перевірки виробника перед оплатою. Файл `database/content/blog/2026_09_26_blog_ru_batch_05.json`; контент зберігається в БД, редагується у звичайній адмінці статей і не змінює українські версії. Загалом наповнено 10 із 20 статей цього розділу.
...
- **Ручки MVM AURA A-2024, HARMONY A-2025, EDGE A-2028, GRIP A-2029, FLOW A-2030 та комплект NEO Z-1319 під WC:** описи останньої 31 картки, по 6 характеристик, 3 FAQ та meta українською і російською. Файл `2026_09_25_mvm_a2024_a2030_z1319_batch_01.json`; матеріал, форму розетки, доступні покриття й геометрію моделей звірено з офіційними картками MVM, склад варіанта — з картками Bona Doors. Після цього наповнено всі 273 картки MVM із плану.
- **Комплектуючі ArtPorte, перша група:** описи 8 карток коробів, лиштви, доборів і плінтусів, по 6 характеристик, 3 FAQ та meta українською і російською. Файл `2026_09_25_artporte_components_batch_01.json`; розміри, основу й покриття коробів, два пази доборів та розміри плінтусів звірено з офіційною технічною інформацією ArtPorte. Наповнено 8 із 94 комплектуючих у плані.
- **Комплектуючі Estet, перша група:** описи 9 карток коробів, лиштви, доборів і плінтуса, по 6 характеристик, 3 FAQ та meta українською і російською. Файл `2026_09_25_estet_components_batch_01.json`; системи, розміри й кольори взято з чинних карток Bona Doors, а непідтверджені матеріали та склад комплектів не додавалися. Для трьох карток лиштви й трьох доборів одночасно замінено дубльовані meta. Наповнено 17 із 94 комплектуючих у плані.
- **Комплектуючі Estet Doors із розмірами:** описи 7 карток коробів, лиштви й доборів, по 6 характеристик, 3 FAQ та meta українською і російською. Файл `2026_09_25_estet_doors_components_batch_01.json`; використано розміри з H1 і slug чинних карток, без непідтверджених матеріалів, кольорів та складу комплектів. У meta двох лиштв виправлено старі помилкові розміри 10×70 і 10×94 мм на 10×80 і 10×84 мм. Наповнено 24 із 94 комплектуючих у плані.
- **Комплектуючі Status RAL 9003:** описи 5 карток телескопічного коробу, лиштви та доборів, по 6 характеристик, 3 FAQ та meta українською і російською. Файл `2026_09_25_status_components_batch_01.json`; для доборів окремо показано розмір чинної картки 2200 мм і технічний розмір Status 2070 мм із вимогою підтвердити фактичну довжину до оплати. Одночасно виправлено переплутані UA/RU описи добору 90 мм та прибрано застарілі дублікати характеристик розміру й кольору. Наповнено 29 із 94 комплектуючих у плані.
- **Комплектуючі Maxi/BonaDoors:** описи 11 карток дверних коробок, лиштви, розширювачів і плінтуса, по 6 характеристик, 3 FAQ та meta українською і російською. Файл `2026_09_25_maxi_components_batch_01.json`; матеріали, системи, розміри та комплектність взято з чинних назв карток Bona Doors, без непідтверджених кольорів, покриттів і кріплень. Наповнено 40 із 94 комплектуючих у плані.
- **Комплектуючі Korfad Exellence:** описи 10 карток телескопічного, компланарного та INSIDE коробів, лиштви й доборів, по 6 характеристик, 3 FAQ та meta українською і російською. Файл `2026_09_25_korfad_exellence_components_batch_01.json`; системи, розміри, фарбоване виконання, строк і гарантію звірено з офіційними картками Korfad, а шість доступних кольорів — із чинними картками Bona Doors. Непідтверджений склад комплектів не додавався. Наповнено 50 із 94 комплектуючих у плані.
- **Стандартні комплектуючі Korfad:** описи 14 карток коробів, лиштви, доборів, плінтуса, з’єднувального профілю та капітелей, по 6 характеристик, 3 FAQ та meta українською і російською. Файл `2026_09_25_korfad_standard_components_batch_01.json`; розміри, матеріали, комплектність і доступні декори взято з чинних карток Bona Doors, а призначення капітелей, строк і гарантію — з офіційного каталогу Korfad. Одночасно виправлено дубльовану російську назву H-подібного профілю. Наповнено 64 із 94 комплектуючих у плані.
- **Комплектуючі Gorgania:** описи 12 карток телескопічних і компланарних коробів та лиштви, доборів, прихованих завіс Anselmi 140 і плінтусів, по 6 характеристик, 3 FAQ та meta українською і російською. Файл `2026_09_25_gorgania_components_batch_01.json`; кольори й назви систем взято з чинних карток Bona Doors, а конструкцію телескопічного коробу, розміри лиштви й доборів та матеріал плінтуса — з офіційного каталогу Gorgania. Для компланарних деталей і плінтусів непідтверджені повні розміри не додавалися. Окремими пакетами наповнено 76 із 94 комплектуючих; з урахуванням п’яти карток MVM із цього переліку, які вже мають контент, готова 81 із 94.
- **Решта комплектуючих:** описи останніх 13 карток прихованих завіс ArtPorte, Otlav, Omega і BonaDoors, магнітного замка AGB Polaris, декоративних елементів Omega, алюмінієвого торця та плінтусів Phoenix, по 6 характеристик, 3 FAQ та meta українською і російською. Файл `2026_09_25_remaining_components_batch_01.json`; розміри, комплектність, кольори й статус узято з чинних карток Bona Doors, а принцип роботи Polaris звірено з офіційним каталогом AGB. Для позицій без точного артикула, навантаження або матеріалу ці дані не вигадувалися. Після цього наповнено всі 94 комплектуючі з плану, включно з п’ятьма картками MVM, що перетинаються з розділом ручок.
- **Унікальні meta для 61 товару:** для останніх 55 технічних і вхідних дверей та фарбованих плінтусів додано окремі `meta_title` і `meta_description` українською та російською з моделлю, розміром або кольором. Файл `2026_09_26_duplicate_meta_batch_01.json`; разом із шістьма картками Estet, виправленими під час наповнення комплектуючих, закрито всі 61 позицію з пріоритету 5.
- **Розсувні системи Häfele Slido, перша група:** описи 7 карток Classic 40–120P, Synchro 40–120P, Telescopic 80–120P, Design 100-S, Design 70V, Design 80V і T-Snap, по 7 характеристик, 4 FAQ та унікальні meta українською і російською. Файл `2026_09_26_slido_sliding_doors_batch_01.json`; навантаження, ширини, монтаж і склад варіантів звірено з технічними матеріалами Häfele. Для карток без точного артикула межі й комплектацію не вигадували. Ціну свідомо залишено «за запитом», оскільки вона залежить від полотна, шини, демпферів, кріплень і монтажу. Наповнено 7 із 20 розсувних систем у плані.
- **Решта розсувних дверей і перегородок:** описи 13 карток Ceiling Mount, Classic, Celegon Ergon Living T.E., Magic, Magic Mirror, Mantion SAF-SLIM та семи конфігурацій Loft, по 7 характеристик, 4 FAQ й унікальні meta українською і російською. Файли `2026_09_26_sliding_doors_batch_02.json` і `2026_09_26_sliding_doors_batch_03.json`; параметри Ergon Living T.E., актуальної Magic2 та SAF-SLIM звірено з офіційними матеріалами Celegon, Terno Scorrevoli й Mantion. Для проєктних дверей без артикула не вигадували вагу, ширину, тип скла або склад комплекту: їх прив’язано до заміру, креслення й письмової специфікації. Ціну залишено «за запитом». Після цього наповнено всі 20 розсувних систем у плані.
- **Інші двері з коротким описом, перша група:** розширені описи 10 карток Artporte Чикаго й Нью-Йорк та Maxi Classic1 NEW, Classic2 NEW, Delicate, Dublin, Elegance, Elegance Wood, Glasso і Lines, по 7 характеристик, 4 FAQ та унікальні meta українською і російською. Файл `2026_09_26_short_description_products_batch_01.json`; конструкцію, розміри й палітру взято з чинних карток Bona Doors, а тип полотна, фрезерування та скло моделей Maxi звірено з офіційним каталогом виробника. Для Elegance Wood не вигадували заповнення великої вставки, якого немає в специфікації, а точні дублікати старих характеристик кольору й розміру безпечно прибираються під час імпорту. Наповнено 10 із 66 товарів цього розділу.
- **Інші двері з коротким описом, друга група:** розширені описи 10 карток Status Dream, Dream Glass і Флоренція та Estet МК База, Діагональ, Горизонталь, Гранд, Прованс, Прованс Glass і Сплінт, по 7 характеристик, 4 FAQ та унікальні meta українською і російською. Файли `2026_09_26_short_description_products_batch_02.json` і `2026_09_26_short_description_products_batch_03.json`; конструкцію, кольори й розміри взято з чинних карток Bona Doors, а тип полотна та скло звірено зі специфікаціями виробників і профільних продавців. Непідтверджені строки, гарантію та склад комплекту не додавали. Наповнено 20 із 66 товарів цього розділу.
- **Інші двері з коротким описом, третя група:** розширені описи 10 карток Status FLY і Geometry та Gorgania Бескид, Дукач, Гердан, Говерла, Грофа, Красія, Магура й Осій, по 7 характеристик, 4 FAQ та унікальні meta українською і російською. Файли `2026_09_26_short_description_products_batch_04.json` і `2026_09_26_short_description_products_batch_05.json`; розміри й декори збережено з чинних карток Bona Doors, а тип полотна, розташування вставок і скління звірено з фото моделей та доступними матеріалами виробників. Непідтверджені строки, гарантію й матеріал окремих декоративних вставок не вигадували. Наповнено 30 із 66 товарів цього розділу.
- **Інші двері з коротким описом, четверта група:** розширені описи 10 карток Gorgania Осій плюс, Осій плюс зі склом, Петрос, Плай, Плай плюс, Руна, Салба, Шелест, Стіг і Стримба, по 7 характеристик, 4 FAQ та унікальні meta українською і російською. Файли `2026_09_26_short_description_products_batch_06.json` і `2026_09_26_short_description_products_batch_07.json`; розміри й кольори збережено з чинних карток Bona Doors, конструкцію, варіанти скління та відмінності моделей звірено з каталогом Gorgania, фото й актуальними картками профільних продавців. Непідтверджені строки та матеріал окремих декоративних вставок не додавали. Наповнено 40 із 66 товарів цього розділу.
- **Інші двері з коротким описом, п’ята група:** розширені описи 10 карток Gorgania Яворина й Яворина плюс, Maxi Molding, Molding 1B, Molding 2B, Parallel та Ultra, Omega Art Vision A1 у шпоні, Status Sky Molding Black і Sky, по 7 характеристик, 4 FAQ та унікальні meta українською і російською. Файли `2026_09_26_short_description_products_batch_08.json` і `2026_09_26_short_description_products_batch_09.json`; старі характеристики кольору й розміру збережено дослівно, скління Яворини звірено з опціями чинних карток Bona Doors, молдинги та тип полотен Maxi — з каталогом продавця виробника, конструкцію Status і обмеження INSIDE для Omega — з чинних карток та специфікацій. Непідтверджені строки, склад внутрішнього заповнення й точні відтінки декоративних вставок не додавали. Наповнено 50 із 66 товарів цього розділу.
- **Інші двері з коротким описом, шоста група:** розширені описи 5 карток Status Ultra-005, Ultra Black Glass 120 мм і Ultra2 Glass Satin 30 мм та Maxi Vertical 1В і Vertical 2В, по 7 характеристик, 4 FAQ та унікальні meta українською і російською. Файл `2026_09_26_short_description_products_batch_10.json`; старі характеристики кольору й розміру збережено дослівно, конструкцію і скління Status звірено з чинними картками та специфікаціями продавців, а розташування канелюр і молдингів Maxi — з фото моделей. Непідтверджені строки, гарантію та матеріал молдингів не додавали. Наповнено 55 із 66 товарів цього розділу.
- **Інші двері з коротким описом, фінальна група:** розширені описи вхідної моделі Qdoors Лаунж-Ак Ультра та 10 технічних дверей Fort-M Техно 1 і Техно База в розмірах 860, 960 і 1200 мм та кольорах RAL 7024 і RAL 8017, по 7 характеристик, 4 FAQ й унікальні meta українською і російською. Файли `2026_09_26_short_description_products_batch_11.json`, `2026_09_26_short_description_products_batch_12.json` і `2026_09_26_short_description_products_batch_13.json`; розміри, товщини, метал, ущільнення, замки та склад базової комплектації збережено з чинних карток Bona Doors і звірено з актуальними картками виробника або профільного продавця. Застарілу фіксовану доплату за лиштву не переносили: вона оформлюється окремою позицією за актуальною ціною. Після цієї групи наповнено всі 66 товарів розділу.
